...
class Snowman: UIView{
    ...
    private var mustacheCenter: CGPoint {
        return CGPoint(x: bounds.midX, y: bounds.midY - 2 * middleRadius + headRadius * 0.1)
    }

    //MARK: Functions for Drawing
    //MARK: BODY
        private func pathForMustache() -> UIBezierPath {
    
            let lineNose = UIBezierPath()
            lineNose.move(to: mustacheCenter)
            lineNose.addLine(to: CGPoint(x: bounds.midX + headRadius * 1.3, y: bounds.midY - 2 * middleRadius + headRadius * 0.35))
            lineNose.addLine(to: CGPoint(x: bounds.midX, y: bounds.midY - 2 * middleRadius + headRadius * 0.2))
            lineNose.addLine(to: CGPoint(x: bounds.midX - headRadius * 1.3, y: bounds.midY - 2 * middleRadius + headRadius * 0.2))
